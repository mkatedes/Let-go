...
void UEMSAsyncStream::StartLoad()
{
	if (EMS)
	{
		...
		//Spawn Loader
		const FLoaderInitData LoaderData = FLoaderInitData(true, PrunedActors, StreamActorsMap);
...
bool UEMSAsyncStream::SetupLevelActors()
{
	...
	for (AActor* Actor : StreamingLevel->Actors)
	{
			...
			&& !FActorHelpers::IsSkipSave(Actor))
		{
			...
			StreamActorsMap.Add(FActorByteKey(EMS->BytesFromString(FullName)), Actor);
		}
	}
